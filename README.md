...
- **🧩 插件中心**：自动扫描 AstrBot 全部插件（元数据/连通状态/配置 schema 自动渲染表单/启禁用）
- **🛒 插件市场**：内置 + 远程索引 (GitHub raw, 镜像加速)，一键安装/卸载 + 依赖自动安装
- **🤖 模型提供商**：可视化管理 AstrBot provider 列表（API Key / 启禁用 / 增删）
- **📚 知识库 / 🧬 人格设定 / ⚙️ 自定义规则**：AstrBot 配置表单化
- **🔑 OpenAPI**：面板 API 密钥生成/撤销，自动化脚本调用面板接口
- **🧩 子代理编排 / 📊 数据统计 / 🐾 追踪日志**：Agent 配置、消息统计、Trace 查看
